...
namespace AMSMigrate.Ams
{
    internal class AssetMigrator : BaseMigrator
    {
        ...
        //public override async Task<MigrationResult> MigrateAsync(string sourceStorageAccountName, string targetStorageAccountName, string containerName, string assetName, CancellationToken cancellationToken)
        //{
        //    var watch = Stopwatch.StartNew();
        //    var stats = new AssetStats();
        //    var sourceStorage = await _resourceProvider.GetStoraAccountByNameAsync(sourceStorageAccountName, cancellationToken);

        //    AssetMigrationResult result = new AssetMigrationResult(MigrationStatus.NotMigrated);
        //    _logger.LogInformation("Migrating asset: {name} (container {container}) ...", assetName, containerName);

        //    try
        //    {
        //        var sourceContainer = sourceStorage.GetBlobContainerClient(containerName);

        //        if (!await sourceContainer.ExistsAsync(cancellationToken))
        //        {
        //            _logger.LogWarning("Container {name} missing for asset {asset}", sourceContainer.Name, assetName);
        //            result.Status = MigrationStatus.Failed;
        //            _logger.LogDebug("Migrated asset: {asset}, container: {container}, type: {type}, status: {status}", assetName, containerName, result.AssetType, result.Status);
        //            return result;
        //        }

        //        // Get the initial migration status from the container level's metadata list.
        //        result = await _tracker.GetMigrationStatusAsync(sourceContainer, cancellationToken);

        //        if (_options.SkipMigrated)
        //        {
        //            if (result.Status == MigrationStatus.Completed)
        //            {
        //                _logger.LogDebug("Asset: {name} has already been migrated.", assetName);
        ...
        //        var details = await sourceContainer.GetDetailsAsyncFromContainer(_logger, assetName, cancellationToken, null);
        ...
        //                    {
        //                        var transformResult = (AssetMigrationResult)await transform.RunAsync(details, sourceContainer, cancellationToken);
        ...
        //            _logger.LogWarning("Skipping asset {name} because it is not in a supported format!!!", assetName);
        //        }

        //        await _tracker.UpdateMigrationStatus(sourceContainer, result, cancellationToken);
        ...
        //    catch (Exception ex)
        //    {
        //        _logger.LogError(ex, "Failed to migrate asset {name}.", containerName);
        //        result.Status = MigrationStatus.Failed;
        //        _logger.LogDebug("Migrated asset: {asset}, container: {container}, type: {type}, status: {status}", assetName, containerName, result.AssetType, result.Status);
        //        return result;
        //    }
