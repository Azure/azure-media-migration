...
namespace AMSMigrate.Ams
{
    internal class AssetAnalyzer : BaseMigrator
    {
        ...
        private async Task<AnalysisResult> AnalyzeAsync<T>(T item, BlobServiceClient storage, CancellationToken cancellationToken)
        {
            ...
            if (!_analysisOptions.IsStorageAcc && item is MediaAssetResource mediaAsset)
            {
            ...
            else if (_analysisOptions.IsStorageAcc && item is BlobContainerItem bcItem)
            {
                itemName = storage.AccountName;
            ...
            try
            {
                ...
                if (migrateResult.Status != MigrationStatus.Completed && migrateResult.Status != MigrationStatus.Failed)
                {
                    ...
                    {
                        _logger.LogWarning("Asset {name} is encrypted", itemName);

                        migrateResult.AssetType = AssetMigrationResult.AssetType_Encrypted;
                    }
                    else
                    {
                        var assetDetails = await container.GetDetailsAsync(_logger, cancellationToken, null, itemName, false);
